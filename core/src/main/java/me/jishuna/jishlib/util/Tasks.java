...
public final class Tasks {
    public static BukkitTask run(Runnable task) {
        return Bukkit.getScheduler().runTask(JishLib.instance(), task);
    }

    public static BukkitTask runAsync(Runnable task) {
        return Bukkit.getScheduler().runTaskAsynchronously(JishLib.instance(), task);
    }

    public static BukkitTask runLater(Runnable task, long ticks) {
        return Bukkit.getScheduler().runTaskLater(JishLib.instance(), task, ticks);
    }

    public static BukkitTask runLaterAsync(Runnable task, long ticks) {
        return Bukkit.getScheduler().runTaskLaterAsynchronously(JishLib.instance(), task, ticks);
    }
    ...
    public static BukkitTask runTimer(Runnable task, long delay, long ticks) {
        return Bukkit.getScheduler().runTaskTimer(JishLib.instance(), task, delay, ticks);
    }
    ...
    public static void runTimer(Consumer<BukkitTask> task, long delay, long ticks) {
        Bukkit.getScheduler().runTaskTimer(JishLib.instance(), task, delay, ticks);
    }
    ...
    public static BukkitTask runTimerAsync(Runnable task, long delay, long ticks) {
        return Bukkit.getScheduler().runTaskTimerAsynchronously(JishLib.instance(), task, delay, ticks);
    }

    public static <T> Future<T> callSync(Callable<T> callable) {
        return Bukkit.getScheduler().callSyncMethod(JishLib.instance(), callable);
    }

    public static <T> CompletableFuture<T> completeSync(Callable<T> callable) {
        return CompletableFuture.supplyAsync(() -> {
            Future<T> future = Bukkit.getScheduler().callSyncMethod(JishLib.instance(), callable);
            try {
                return future.get();
            } catch (Exception e) {
